...
const STORAGE_KEYS = {
  ...
  // INITIALIZED key removed as we no longer need to track one-time mock injection
};
...
export const App: React.FC = () => {
  ...
  // The useEffect that previously loaded "Korea Trip" and "Check flights" has been deleted.
  // The app will now start clean if localStorage is empty.
          ...
          reader.onloadend = async () => {
              ...
              try {
                  ...
                  if (compressed.length > 2 * 1024 * 1024) {
                      alert("Image is still quite large and might take a moment to process.");
                  }
    ...
    try {
      ...
      if (isGoal && inputText.length < 100 && !inputText.includes('/') && !selectedImage) {
        ...
        const newMemory: Memory = {
          ...
          attachedImage: selectedImage || undefined
        };
      ...
      if (errMsg.includes("413") || errMsg.includes("Too Large")) {
          alert("Input too large! Please use a smaller image or shorter text.");
      } else if (errMsg.includes("API_KEY")) {
          alert("API Key missing or invalid. Check your environment variables.");
      } else {
          alert(`Failed to process input: ${errMsg}. Check console for details.`);
      }
    } finally {
  ...
  const handleDeleteCategory = (cat: string) => {
      if(availableCategories.length <= 1) { alert("Must have at least one category."); return; }
  ...
  const handleAddProject = (targetCategory?: string) => {
    ...
    if (!isProjectNameGloballyUnique(trimmedName)) {
        alert(`Project "${trimmedName}" already exists. Project names must be unique.`);
        return;
    }
  ...
  const handleUpdateProjectName = (oldName: string, newName: string) => {
      if (!newName.trim() || newName === oldName) return;
      if (!isProjectNameGloballyUnique(newName.trim())) { alert("Project name already exists."); return; }
  ...
  const handleSearch = async () => {
    ...
    } catch (e) { setSearchResult("Error occurred."); } 
  ...
  const renderHighlightedText = (text: string) => {
      ...
      return parts.map((part, index) => {
          if (part.startsWith('/')) {
              return (
                  <span key={index} className="bg-indigo-50 text-indigo-600">
                      {part}
                  </span>
              );
          }
  ...
  const renderInputView = () => {
      ...
      const filteredSuggestions = tagSearchTerm 
        ? allTags.filter(t => t.toLowerCase().includes(tagSearchTerm.toLowerCase()))
        : allTags;

      const placeholderText = "Capture ideas, upload screenshots, or plan your next move.";
      ...
      return (
        ...
        <div className="flex-1 relative">
            <div className={`relative w-full h-64 border-2 rounded-2xl bg-white transition-all shadow-sm overflow-hidden flex flex-col ${selectedImage ? 'border-indigo-500' : 'border-slate-100 focus-within:border-indigo-500'}`}>
                ...
                <div className="relative flex-1 group">
                    <div 
                        ref={backdropRef}
                        className="absolute inset-0 p-4 text-lg font-sans leading-relaxed tracking-normal whitespace-pre-wrap break-words pointer-events-none text-transparent overflow-hidden"
                        aria-hidden="true"
                    >
                    ...
                    <textarea
                        ...
                        className="absolute inset-0 w-full h-full p-4 bg-transparent border-none outline-none resize-none text-lg font-sans leading-relaxed tracking-normal text-transparent caret-slate-800 z-10 placeholder:text-slate-300"
            ...
            {showTagSuggestions && filteredSuggestions.length > 0 && (
                <div 
                    ref={suggestionsRef}
                    className="absolute z-50 bg-white/95 backdrop-blur-sm border border-slate-200 shadow-2xl rounded-xl p-2 animate-in fade-in zoom-in-95 w-72"
                    style={{
                        top: Math.min(suggestionPos.top, 250), 
                        left: Math.min(suggestionPos.left, 150) 
                    }}
                >
                    <div className="text-xs font-bold text-slate-400 mb-2 px-2 uppercase tracking-wider">
                        {tagSearchTerm ? 'Matching labels' : 'Suggestions'}
                    </div>
                    <div className="flex flex-col gap-1 max-h-48 overflow-y-auto no-scrollbar">
                        {filteredSuggestions.map((tag, idx) => (
                            <button 
                                ...
                                className="px-3 py-2.5 bg-slate-50 hover:bg-indigo-50 hover:text-indigo-700 rounded-lg text-sm font-medium flex items-center space-x-3 text-left transition-colors border border-transparent hover:border-indigo-100"
                            >
                                <Tag className="w-3.5 h-3.5 flex-shrink-0 opacity-70" />
        ...
        <div className="mt-6">
            <button onClick={handleProcessInput} disabled={isProcessing || (!inputText.trim() && !selectedImage) || isCompressingImage} className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 text-white font-bold py-4 rounded-xl flex items-center justify-center space-x-2 shadow-lg shadow-indigo-200 transition-all">
                {isProcessing ? <><Loader2 className="w-5 h-5 animate-spin" /><span>Processing...</span></> : <><Sparkles className="w-5 h-5" /><span>Process Input</span></>}
            </button>
        </div>
        </div>
      );
  ...
  const renderTodayView = () => {
    ...
    const monthOptions = [];
    for (let y = 2025; y <= 2026; y++) { for (let m = 0; m < 12; m++) monthOptions.push(new Date(y, m, 1)); }
                        ...
                        <>
                            ...
                            <div className="absolute top-full left-0 mt-3 w-64 bg-white rounded-2xl shadow-xl border border-slate-100 max-h-80 overflow-y-auto z-50 no-scrollbar">
                                <div className="p-2 space-y-1">
                                    {monthOptions.map(d => (
                                        <button key={d.toISOString()} onClick={() => { setSelectedDate(d); setIsMonthPickerOpen(false); }} className={`w-full text-left px-4 py-3 rounded-xl text-sm font-medium ${isSelectedDate(d, selectedDate) ? 'bg-indigo-50 text-indigo-700 font-bold' : 'text-slate-600 hover:bg-slate-50'}`}>
        ...
        <div className={`overflow-hidden transition-all duration-300 ${isCalendarOpen ? 'max-h-80 border-b border-slate-100' : 'max-h-0'}`}>
            <div className="p-4 pt-0">
                <div className="grid grid-cols-7 gap-1 text-center mb-3">
                    {['S','M','T','W','T','F','S'].map(d => <div key={d} className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{d}</div>)}
                </div>
  ...
  const renderMemoryView = () => {
    // ... (Keep existing Memory View Logic)
    ...
    filteredMemories.forEach(memory => {
        const proj = memory.project || 'General';
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 relative">
          ...
          <div className="p-6 bg-white sticky top-0 z-10 shadow-sm">
            <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex space-x-2">
                    <button onClick={() => { setShowCategoryManager(true); setManagerSelectedCategory(null); }} className="flex items-center space-x-1 px-3 py-1.5 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 font-medium text-xs transition-colors">
                        <Settings className="w-4 h-4" /><span>Structure</span>
                    </button>
                </div>
            </div>
            <div className="relative mb-4">
                <Search className="absolute left-3 top-3 w-5 h-5 text-slate-400" />
                <input type="text" placeholder="Search memories..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch()} className="w-full pl-10 pr-4 py-3 bg-slate-100 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-slate-800" />
            ...
            <div className="flex space-x-2 overflow-x-auto no-scrollbar pb-1 items-center">
                <button onClick={() => setShowAddProjectModal(true)} className="px-2.5 py-1.5 rounded-full bg-slate-100 hover:bg-indigo-50 text-indigo-600 transition-colors flex-shrink-0" title="Add New Project"><Plus className="w-4 h-4" /></button>
                <div className="h-5 w-px bg-slate-200 mx-1 flex-shrink-0"></div>
                <button onClick={() => setSelectedRootCategory('All')} className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors flex-shrink-0 ${selectedRootCategory === 'All' ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-500'}`}>All</button>
                {availableCategories.map(cat => (
                    <button key={cat} onClick={() => setSelectedRootCategory(cat)} className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors flex-shrink-0 ${selectedRootCategory === cat ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-500'}`}>{cat}</button>
                ))}
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 pb-24 no-scrollbar">
            {searchResult && (
                <div className="mb-8 bg-indigo-50 p-4 rounded-xl border border-indigo-100 animate-in fade-in">
                    <div className="flex items-center space-x-2 mb-2 text-indigo-700 font-bold text-sm"><Sparkles className="w-4 h-4" /><span>AI Answer</span></div>
            ...
            {Object.keys(groupedData).length === 0 ? <div className="text-center py-20 opacity-50"><p>No memories found.</p></div> : (
                <div className="space-y-3">
                    {/* General Items */}
                    {groupedData['General'] && (
                        <div className="mb-6 space-y-3">
                             {Object.values(groupedData['General']).map(mems => mems.flatMap((m, i) => m.structuredContent.map((item, idx) => (
                                    <MemoryCard key={`gen-${m.id}-${idx}`} item={item} displayDate={item.targetDate || formatDate(m.createdAt)} attachedImage={m.attachedImage} onDelete={() => handleDeleteMemoryItem(m.id, idx)} onUpdate={(u) => handleUpdateMemoryItem(m.id, idx, u)} />
                             ))))}
                        </div>
                    )}
                    {/* Projects */}
                    {Object.keys(groupedData).filter(p => p !== 'General').map(projectName => {
                        ...
                        return (
                            <div key={projectName} className="bg-white rounded-xl overflow-hidden border border-slate-200 shadow-sm">
                                <button onClick={() => toggleFolder(projectName)} className="w-full flex items-center justify-between p-4 bg-white hover:bg-slate-50 transition-colors">
                                    <div className="flex items-center space-x-3">
                                        {isExpanded ? <FolderOpen className="w-6 h-6 text-indigo-500 fill-indigo-50" /> : <Folder className="w-6 h-6 text-slate-400 fill-slate-50" />}
                                        <span className="font-bold text-slate-800">{projectName}</span>
                                        <span className="text-xs text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{Object.values(subProjects).reduce((acc, curr) => acc + curr.length, 0)} items</span>
                                    </div>
                                    ...
                                    <div className="bg-slate-50 border-t border-slate-100 p-3 space-y-4">
                                        {Object.keys(subProjects).map(subProjectName => (
                                            <div key={subProjectName}>
                                                {(subProjectName !== 'General' || Object.keys(subProjects).length > 1) && <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">{subProjectName}</h4>}
                                                <div className="grid gap-3">
                                                    {subProjects[subProjectName].flatMap(mem => mem.structuredContent.map((item, idx) => (
                                                        <MemoryCard key={`${mem.id}-${idx}`} item={item} displayDate={item.targetDate || formatDate(mem.createdAt)} attachedImage={mem.attachedImage} onDelete={() => handleDeleteMemoryItem(mem.id, idx)} onUpdate={(u) => handleUpdateMemoryItem(mem.id, idx, u)} />
                                                    )))}
                                                </div>
                                            </div>
                                        ))}
          ...
          {showAddProjectModal && (
            <div className="absolute inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl">
                    <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold text-slate-800">Create New Project</h3><button onClick={() => setShowAddProjectModal(false)}><X className="w-6 h-6 text-slate-400" /></button></div>
                    <div className="space-y-4">
                        <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Category</label><div className="flex flex-wrap gap-2">{availableCategories.map(cat => <button key={cat} onClick={() => setNewProjectCategory(cat)} className={`px-3 py-1 rounded-md text-xs font-medium border ${newProjectCategory === cat ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200'}`}>{cat}</button>)}</div></div>
                        <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Project Name</label><input type="text" value={newProjectName} onChange={(e) => setNewProjectName(e.target.value)} className="w-full p-3 bg-slate-50 border rounded-xl outline-none" autoFocus /></div>
                        <button onClick={() => handleAddProject()} disabled={!newProjectName.trim()} className="w-full bg-indigo-600 text-white font-bold py-3.5 rounded-xl mt-4 disabled:opacity-50">Create Project</button>
                    </div>
                </div>
            </div>
          )}
          {showCategoryManager && (
              <div className="absolute inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
                  <div className="bg-white rounded-2xl w-full max-w-2xl h-[80vh] shadow-2xl flex overflow-hidden">
                      <div className="w-1/3 bg-slate-50 border-r border-slate-200 p-4 flex flex-col">
                          ...
                          <div className="flex-1 overflow-y-auto space-y-2 no-scrollbar">{availableCategories.map(cat => <div key={cat} onClick={() => setManagerSelectedCategory(cat)} className={`p-3 rounded-lg cursor-pointer flex justify-between items-center group ${managerSelectedCategory === cat ? 'bg-white shadow-sm text-indigo-700' : 'hover:bg-slate-200'}`}><span className="text-sm font-medium">{cat}</span><button onClick={(e) => {e.stopPropagation(); handleDeleteCategory(cat)}} className="opacity-0 group-hover:opacity-100 hover:text-red-500"><Trash2 className="w-3 h-3" /></button></div>)}</div>
                          <div className="pt-4 border-t"><input value={tempCategoryName} onChange={(e) => setTempCategoryName(e.target.value)} placeholder="New Category" className="w-full p-2 border rounded mb-2 text-sm"/><button onClick={handleAddCategory} disabled={!tempCategoryName.trim()} className="w-full bg-slate-800 text-white py-2 rounded text-xs font-bold disabled:opacity-50">Add</button></div>
                      </div>
                      <div className="w-2/3 p-6 flex flex-col bg-white">
                         <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold text-slate-800">{managerSelectedCategory || 'Select Category'}</h3><button onClick={() => setShowCategoryManager(false)}><X className="w-5 h-5 text-slate-400"/></button></div>
                         {!managerSelectedCategory ? <div className="flex-1 flex items-center justify-center text-slate-400 text-sm">Select a category</div> : (
                             <>
                                <div className="flex-1 overflow-y-auto space-y-2 pr-2 no-scrollbar">
                                    <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-lg text-indigo-700 text-sm font-medium flex items-center"><Layers className="w-4 h-4 mr-2"/>General</div>
                                    {getProjectHierarchy()[managerSelectedCategory]?.map(proj => <div key={proj} className="p-3 border rounded-lg flex justify-between items-center group"><div className="flex items-center space-x-3"><Folder className="w-4 h-4 text-slate-400"/><input defaultValue={proj} onBlur={(e) => handleUpdateProjectName(proj, e.target.value)} className="text-sm font-medium outline-none"/></div><button onClick={() => handleDeleteProject(proj)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100"><Trash2 className="w-4 h-4"/></button></div>)}
                                </div>
                                <div className="pt-6 border-t mt-4"><label className="text-xs font-bold text-slate-400 uppercase mb-2 block">Add Project</label><div className="flex space-x-2"><input value={tempProjectName} onChange={(e) => setTempProjectName(e.target.value)} placeholder="Project Name" className="flex-1 p-2.5 bg-slate-50 border rounded-xl text-sm outline-none"/><button onClick={() => handleAddProject(managerSelectedCategory)} disabled={!tempProjectName.trim()} className="px-4 bg-indigo-600 text-white font-bold rounded-xl text-sm disabled:opacity-50">Add</button></div></div>
                             </>
                         )}
                      </div>
                  </div>
  ...
  const isSelectedDate = (d1: Date, d2: Date) => d1.getFullYear() === d2.getFullYear() && d1.getMonth() === d2.getMonth();

  return (
    <div className="max-w-md mx-auto bg-white h-screen shadow-2xl overflow-hidden flex flex-col font-sans">
       {!import.meta.env.VITE_GEMINI_API_KEY && <div className="bg-red-500 text-white text-xs p-1 text-center font-bold">WARNING: API_KEY is missing.</div>}
